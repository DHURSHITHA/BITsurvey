...
const SurveyCreation = () => {
  ...
  const fetchDraftSurvey = async (survey_id) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/get-draft-survey/${survey_id}`, {
        method: "GET",
        headers: {
          Authorization: token,
        },
  ...
  const handlePermissionData = async (e) => {
    ...
    if (name === "assignedRoles") {
      if (value === "Year") {
        ...
        try {
          const token = localStorage.getItem("token");
          const response = await axios.get("http://localhost:3000/get-faculty-groups", {
            headers: {
              Authorization: token,
            },
          });
  ...
  const savePermissionDataToDatabase = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(
        "http://localhost:3000/save-permissions",
        {
          ...permissionData,
          surveyTitle,
        },
            ...
            {assignRolesChecked && (
              <div style={{ marginLeft: "20px", width: "200px" }}>
                ...
                >
                  ...
                  <MenuItem onClick={handleYearMenuOpen}>Year</MenuItem>
                  <MenuItem onClick={handleDepartmentMenuOpen}>Department</MenuItem>
